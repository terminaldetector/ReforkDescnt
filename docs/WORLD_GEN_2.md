...
## LLOD — Long Level of Detail (`world/llod`, `client/llod`)

| Band | Дистанция | Представление |
|------|-----------|---------------|
| FULL | &lt; 256 | Полные блоки |
| MEDIUM | 256–2048 | Промежуточная геометрия / плотные силуэты |
| SILHOUETTE | 2048–65536 | Крупные воксельные силуэты |
| NONE | дальше | Culled |

Сервер шлёт каталог макрообъектов (~2 Hz). Клиент рисует полупрозрачные AABB в `AFTER_TRANSLUCENT`.

`/d6 llod` — принудительный sync + размер каталога.

## Стриминг (`WorldRules.StreamLevel`)

```
MACROWORLD → REGION → CHUNK → LOCAL
```

Мегаструктуры существуют в макрокаталоге независимо от vanilla chunk rings — основа для высокоскоростного полёта без «пустого горизонта».
