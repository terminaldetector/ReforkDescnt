...
if CLIENT then
    ...
    -- ── Панель в Q-меню: DRMD Workshop → Внешний вид оружия ──
    ...
    local function CollectClasses()
        ...
        if D6_WEPVIEW_CFG then
            for cn in pairs(D6_WEPVIEW_CFG) do set[cn] = true end
        end
        for cn in pairs(D6_SCKBridge.Configs) do set[cn] = true end
        local list = {}
        for cn in pairs(set) do table.insert(list, cn) end
        table.sort(list)
        return list
    end

    hook.Add("AddToolMenuTabs", "D6_SCKBridge", function()
        spawnmenu.AddToolTab("DRMD Workshop", "DRMD Workshop", "icon16/bomb.png")
    end)

    hook.Add("AddToolMenuCategories", "D6_SCKBridge", function()
        spawnmenu.AddToolCategory("DRMD Workshop", "VisualOverride", "Внешний вид оружия")
    end)

    hook.Add("PopulateToolMenu", "D6_SCKBridge", function()
        spawnmenu.AddToolMenuOption("DRMD Workshop", "VisualOverride",
            "d6_visual_override", "Визуальные переопределения", "", "", function(panel)
            panel:ClearControls()

            panel:Help("Редактирование внешнего вида существующего оружия "
                .. "через редактор кластеров SCK. Нужно оружие SCK в руках.")

            local combo = panel:ComboBox("Класс оружия")
            for _, cn in ipairs(CollectClasses()) do
                combo:AddChoice(cn, cn, cn == "weapon_d6_mg")
            end

            local function SelectedClass()
                local _, v = combo:GetSelected()
                return v
            end

            local status = panel:Help("")
            local function UpdateStatus()
                local class = SelectedClass()
                if not class then status:SetText("Класс не выбран.") return end
                local cfg = D6_SCKBridge.Configs[class]
                if cfg then
                    local nMuzzles = table.Count(cfg.muzzles or {})
                    status:SetText(("Переопределение активно: %d модулей, %d точек выстрела.")
                        :format(#cfg.modules, nMuzzles))
                elseif D6_WEPVIEW_CFG and D6_WEPVIEW_CFG[class] then
                    status:SetText("Без переопределения (рендер d6_wepview, "
                        .. #D6_WEPVIEW_CFG[class] .. " слотов).")
                else
                    status:SetText("Без переопределения и без слотов wepview.")
                end
            end
            combo.OnSelect = UpdateStatus
            timer.Simple(0, UpdateStatus)

            local loadBtn = panel:Button("Загрузить вид в редактор SCK")
            loadBtn.DoClick = function()
                ...
                if not (IsValid(sck) and sck.IsSCK) then
                    ...
                    chat.AddText(Color(255, 200, 80),
                        "[D6 Bridge] Возьми в руки SCK (выдан) и нажми кнопку ещё раз.")
                    return
                end
                ...
                elseif D6_WEPVIEW_CFG and D6_WEPVIEW_CFG[class] then
                    ...
                    chat.AddText(Color(255, 80, 80),
                        "[D6 Bridge] У класса нет ни переопределения, ни слотов wepview.")
                    return
                end

                sck.clusters = D6_SCKBridge.ModulesToClusters(modules)
                sck._d6BridgeClass = class
                sck:CleanMenu()
                sck:OpenMenu()
                chat.AddText(Color(80, 255, 80),
                    "[D6 Bridge] " .. class .. " загружен. Вкладка Clusters → правь → «Применить».")
            end

            local applyBtn = panel:Button("Применить кластеры из SCK к классу")
            applyBtn.DoClick = function()
                ...
                if not (IsValid(sck) and sck.IsSCK and istable(sck.clusters)) then
                    chat.AddText(Color(255, 80, 80),
                        "[D6 Bridge] Возьми SCK с настроенными кластерами.")
                    return
                end
                local modules = D6_SCKBridge.ClustersToModules(sck.clusters)
                if #modules == 0 then
                    chat.AddText(Color(255, 80, 80), "[D6 Bridge] Кластеры пусты.")
                    return
                end
                D6_SCKBridge.Submit(class, modules)
                timer.Simple(0.3, UpdateStatus)
            end
            panel:ControlHelp("Сохраняется на сервере и применяется всем сразу (нужен admin)")

            local clearBtn = panel:Button("Сбросить переопределение класса")
