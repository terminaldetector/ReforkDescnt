...
- Щиты с регеном
- 28 оружий Descent/Doom-стиля (включая заряжаемую Fusion Cannon)
...
|-----------|------|------------|
| 28 оружий | `AAB / CRB / A··` | `B` — ствол, `C` — ядро (iron → tnt → energy cell → targeting core → nether star) |
...
нестабильный реактор, генератор гравитации, гравифакел, лазерная буровая установка.
Всего **59 рецептов** — покрыты все предметы и блоки, кроме creative-only spawn eggs. Рецепты открываются в книге при
входе в мир.
...
Портирована **Fusion Cannon** из Descent — единственное оружие мода, которое
заряжается удержанием: 2 с до полного заряда, урон 45 → 195, отдача 40 → 320.
...
**Гравифакел** — ходьба по стенам в духе Prey 2006 — теперь есть и на Bedrock,
с теми же константами, что на ПК: радиус 8, захват 0.4 с, отпускание 0.6–0.7 с,
скорость ходьбы 0.219 бл/тик против ванильных 0.216. Подробности и числа —
[`docs/GRAVITY_TORCH.md`](docs/GRAVITY_TORCH.md).

Чего на Bedrock нет: камера не кренится — скриптовый API даёт только yaw/pitch,
поэтому крен читается по приборам, а стену нельзя показать как пол (ходьба по ней
при этом работает полностью). И высоту мира переопределить нельзя, так что
уровни `NETHER / ABYSS / ORBITAL / END` там подписаны, но недостижимы.

### Полёт в креативе больше не залипает

Полёт не работал в креативе ни на одной из платформ — пилот висел невесомым на
точке спавна. Причины оказались разные и складывались:

- **ПК:** скорость выставлялась только на серверной сущности, а игрок никогда не
  входит в собственный трекер — вектор доходил до всех клиентов, кроме пилотского.
- **ПК:** интегратор считает в блоках в **секунду**, а результат уходил в
  `setVelocity`, который в блоках за **тик**. Корпус разгонялся до 19.4 бл/тик —
  388 бл/с, полный чанк за 0.82 тика. Теперь 0.97 в крейсере и 2.8 на форсаже.
- **ПК:** ванильный креативный полёт переписывал Y как `предыдущий * 0.6` каждый
  тик. 6DoF забирает `travel` себе и держит `abilities.flying` выключенным;
  `allowFlying` не трогается, так что `H` мгновенно возвращает полёт для стройки.
- **MCPE:** `applyKnockback` в креативе — молчаливый no-op, а не ошибка. Полёт
  там идёт телепортом с `checkForBlocks`.
