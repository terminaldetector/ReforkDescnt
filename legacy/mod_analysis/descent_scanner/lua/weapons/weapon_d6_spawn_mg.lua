...
-- =========================================================
SWEP.PrintName      = "D6: Сканер-пулемётчик"
SWEP.Author         = "Descent 6DOF"
SWEP.Instructions   = "ЛКМ: заспавнить   ПКМ: сразу 3"
...
local function SpawnMG(ply, pos)
    if SpawnEnemyDrone then SpawnEnemyDrone(ply,"npc_cscanner","mg")
        ...
        npc:SetMaxHealth(120); npc:SetHealth(120)
        npc:SetColor(Color(255,40,40)); npc.D6_Variant="mg"
...
if SERVER then
    function SWEP:DoSpawn(ply, pos) SpawnMG(ply, pos) end
    function SWEP:DoSecondary(ply)
        if CurTime()<(self.NextAlt or 0) then return end
        self.NextAlt=CurTime()+5
        local p=ply:GetPos(); local sa=ply:GetAimVector()
        for i=1,3 do
            SpawnMG(ply, p+sa*200+Vector(math.Rand(-60,60),math.Rand(-60,60),0))
        end
    end
end
