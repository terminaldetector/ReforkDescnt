...
-- =========================================================
SWEP.PrintName      = "D6: Лазерный сканер"
SWEP.Author         = "Descent 6DOF"
SWEP.Instructions   = "ЛКМ: заспавнить   ПКМ: 3 лазерных сканера"
...
local function SpawnLaser(ply, pos)
    if SpawnEnemyDrone then SpawnEnemyDrone(ply,"npc_cscanner","laser")
        ...
        npc:SetColor(Color(0,255,255)); npc.D6_Variant="laser"
...
if SERVER then
    function SWEP:DoSpawn(ply, pos) SpawnLaser(ply, pos) end
    function SWEP:DoSecondary(ply)
        ...
        for i=1,3 do SpawnLaser(ply,p+sa*200+Vector(math.Rand(-70,70),math.Rand(-70,70),0)) end
    end
end
